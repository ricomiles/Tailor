...
- **Bootstrap is idempotent.** One startup routine creates the data directory, seeds `./data/resume.canon.json` from the checked-in seed at `adapters/db/seed/resume.canon.seed.json` *only if absent*, creates the boards file with its documented shape, and applies versioned migrations. Repeated starts modify nothing. Schema is never synchronized by push — a converging push could drop a column holding real history.
- **Migrations ship the mechanism, not the tables.** `postings`, `runs`, `run_steps`, `diff_items`, and `answers` are each created by the later story that first needs them.
...
- **The render-readiness gate returns a list, not a boolean.** It reports *every* reason a run cannot render **or be approved**: an unfilled placeholder in a selected `needs-number` bullet, and a selected `needs-content` bullet. An incomplete contact line is explicitly **not** a blocker. A non-empty list refuses the render and is returned to the caller.
- **Preview and PDF are one component.** One `ResumeDocument`, one shared props-builder. A second resume template anywhere in the codebase breaks the premise that the preview is trustworthy.
- **The text layer is non-negotiable.** After generating, assert the PDF carries extractable text and fail loudly naming the missing layer. Never return a path as though the render succeeded. Output lands under `./out`.
- **Absent contact fields vanish cleanly** — no label, no separator, no dangling punctuation, and no unfilled-field sentinel reaching output. Preview and PDF must omit identically.
- **One error envelope everywhere** — stable machine-readable code, human-readable message, and the failing stage where one applies, structurally identical across every endpoint.
- **Mock content is quarantined.** Invented employers and bullets live behind `USE_MOCK_DATA` for local UI work only: unreachable when the flag is off, and structurally unable to reach the seeded canon under any setting.

## Technical Decisions

- **Hexagonal core.** No file under `core/` may import from `app/`, `adapters/`, `next/*`, `drizzle-orm`, `playwright`, or any Node built-in. Enforced by lint or type-check, and it must *block the build*, not warn. `core/` currently holds `ports/`, `canon/`, `pipeline/`, `validation/`, `diff/`, `scoring/`, `gates/`, `errors/`, `boards/`, and `bootstrap/`; siblings are `adapters/`, `app/` (with `api/` as the composition root), `components/resume-document/`, plus gitignored `data/` and `out/`.
- **Ports are core-defined interfaces; adapters are selected in route handlers, never inside the core.** `BoardPort`, `AtsPort`, `ModelPort`, `RenderPort`, `RepositoryPort` — named for the capability, not the vendor. This epic implements `RenderPort` (HTML → Playwright Chromium → PDF) and the database repository.
- **Declare every cross-unit type once, in the core**, as a named schema with its inferred TypeScript type; every boundary parses through it. This epic owns at least the error envelope, the readiness-blocker list, and the props-builder input type, and no unit may restate or structurally duplicate one. This is the epic's highest-value invariant — later epics built against unnamed shapes is exactly the failure mode being prevented.
...
- **`ResumeDocument` is pure** — no hooks, no store access, no client-only APIs — a function of fully-resolved props. Its props come from one builder whose single named input type carries `basics`, role company/position/dates, education, and skills. (Distinct from the checks projection Epic 4 owns; both declared once.)
- **The canon gateway normalizes asymmetrically.** Canon's unfilled-field sentinel — the literal string `TODO` — is normalized to absent for *scalar `basics` fields only*. A placeholder token inside a bullet's `text` is returned verbatim — later epics require showing it unchanged.
...
- **Type.** Archivo at 400/600/800 only, loaded via Google Fonts. The app runs a deliberately denser scale than the design system's demo scale. `font-variant-numeric: tabular-nums` on all scores, dates, and timers.
- **Structural rules are absolute.** Zero corner radius anywhere. Flush-left everything, including labels inside wide buttons. Strong 2px dividers between major sections — never softened to a hairline or replaced by whitespace.
...
- **Accent is used sparingly** — primary action and small emphasis only. The accent-to-ground pair is tuned to 3:1, so paragraph-size accent text uses the 700 step, not the raw accent.
...
- **The resume document's own proportions belong here**, not to the later review screen: name Archivo 800 23px at `-0.03em`, 10.5px contact line, a 2px ink rule, section headers 9.5px uppercase at `.14em` over a 1px `neutral-400` rule, org lines Archivo 800 11.5px with right-aligned tabular dates above an italic role line, em-dash bullets at 10.5px/1.45 — plus a dashed page boundary at the bottom edge with `page 1 of 1 · letter` and the percentage of page used beneath it.
